SELECT tt1.region, tt1.silver_single_rate, tt2.mix_rate
FROM (SELECT kr.name_short as region, t1.silver_single_rate
		FROM (SELECT hs.region_name,
			   		(teen.total / (sum(hs.people_m) + sum(hs.people_f)))*100 as silver_single_rate
