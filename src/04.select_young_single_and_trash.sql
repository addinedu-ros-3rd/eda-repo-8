SELECT tt1.region, tt1.young_single_rate, tt2.mix_rate
FROM (SELECT kr.name_short as region, t1.young_single_rate
		FROM (SELECT hs.region_name,
			   		(teen.total / (sum(hs.people_m) + sum(hs.people_f)))*100 as young_single_rate
				from (select region_name, (people_m + people_f) as total
						from home_single
						where age_group in (20, 30)
